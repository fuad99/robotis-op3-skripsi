...
void imageCallback(const sensor_msgs::ImageConstPtr& msg)
{
  ...
  try
  {
    ...
    // cv_bridge::CvImagePtr cv_ptr1 = cv_bridge::toCvCopy(msg, "bgr8");
    // cv_bridge::CvImagePtr cv_ptr2 = cv_bridge::toCvCopy(msg, "bgr8");
    
    cv::cvtColor(cv_ptr->image, cv_ptr->image, CV_BGR2HSV);
    // cv::cvtColor(cv_ptr1->image, cv_ptr1->image, CV_BGR2HSV);
    cv::blur( cv_ptr->image, cv_ptr->image, Size(5,5));
    // cv::blur( cv_ptr1->image, cv_ptr1->image, Size(5,5));
    cv::inRange(cv_ptr->image,Scalar(19, 62, 162),Scalar(50, 255, 255),cv_ptr->image); //kuning
    // cv::inRange(cv_ptr->image,Scalar(0, 140, 90),Scalar(179, 255, 255),cv_ptr->image); //merah
    // cv::inRange(cv_ptr->image,Scalar(50, 66, 64),Scalar(70, 255, 255),cv_ptr->image); //hijau
    // cv::inRange(cv_ptr1->image,Scalar(0, 0, 140),Scalar(80, 255, 255),cv_ptr1->image);
    // cv_ptr2->image=(cv_ptr->image+cv_ptr1->image);
    // cv::add(cv_ptr->image,cv_ptr1->image,cv_ptr2->image);
    cv::Canny(cv_ptr->image, cv_ptr->image, 5, 130, 3);
	...
	ros::Time time_st = ros::Time::now ();
 	combined.header.frame_id = "laser_link";
...
int main(int argc, char **argv)
{
  ...
  image_transport::Subscriber sub = it.subscribe("/usb_cam/image_rect_color", 1, imageCallback,hints);
